...
from portal.models import StudentDetails,CompanyDetails,JobPosition, AppliedJobs
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.user.groups.filter(name='student').exists():
             return render(request, 'portal/stulog.html', {'form': form})
            else:
                logout(request)
...
def  company_login(request):
    if request.user.is_authenticated and request.user.groups.filter(name='company').exists():
        return render(request,'portal/comlog.html')
    if request.method == 'POST':
        form = AuthenticationForm(data=request.POST)
        if form.is_valid():
            user = form.get_user()
            ...
            if request.user.groups.filter(name='company').exists():
             return render(request, 'portal/comlog.html', {'form': form})
            else:
                logout(request)
                return render(request, 'portal/company_login.html', {'form': form})
        else:
            return render(request, 'portal/company_login.html', {'form': form})
    else:
        form = AuthenticationForm()
...
def ccd(request):
 if  request.user.is_authenticated and request.user.groups.filter(name='company').exists():
    if request.method == "POST":
            ...
            if form.is_valid():
                stu = request.user.username
                post = CompanyDetails.objects.filter(username=stu)
        ...
        z=post[0].about
        form=ccdForm()
        return render(request, 'portal/ccd.html', {'form': form,'x':x,'y':y,'z':z})
 else:
     return HttpResponse("<h1>u r not logged in</h1>")
    ...
    if request.user.is_authenticated and request.user.groups.filter(name='company').exists():
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                return render(request, 'portal/jobpos.html', {'form': form})
        else:
            form = jobposForm()
            x = request.user.username
            y = CompanyDetails.objects.filter(username=x)
            ...
            return render(request, 'portal/jobpos.html', {'form': form,'x':x,'y':y1})
    else:
        return HttpResponse("<h1>u r not logged in</h1>")
...
def jd(request):
    if request.user.is_authenticated and request.user.groups.filter(name='company').exists():
        if request.method == "POST":
                    s=""
                    print("Hi")
                    book=JobPosition.objects.filter(job_id=request.POST.get("job_id"))
                    print(len(book))
                    if(len(book)!=1):
                        s="wrong job id try again"
                        return render(request, 'portal/jd.html',{'s':s})
                    book=book[0]
                    ...
                    book.save()
                    return render(request,'portal/comlog.html',{'s':s})
            ...
            return render(request, 'portal/jd.html', {'x':x,'y':y1})
    else:
        return HttpResponse("<h1>u r not logged in</h1>")
...
def deletevacan(request):
    if request.user.is_authenticated and request.user.groups.filter(name='company').exists():
        if request.method == "POST":
                    s=""
                    book=JobPosition.objects.filter(job_id=request.POST.get("jobid"))
                    print(len(book))
                    if(len(book)!=1):
                        s="wrong job id try again"
                        return render(request, 'portal/jobdelete.html',{'s':s})
                    AppliedJobs.objects.filter(job_id=book[0]).delete()
                    book[0].delete()
            ...
            return render(request, 'portal/jobdelete.html')
    else:
        return HttpResponse("<h1>u r not logged in</h1>")
...
def viewpos(request):
    if request.user.is_authenticated and request.user.groups.filter(name='company').exists():
        x = request.user.username
        y = JobPosition.objects.filter(username=x)
        s=""
        print(y)
        if(len(y)==0):
            s="no vacancies posted"
        return render(request, 'portal/viewpos.html',{'y':y,'s':s})
    else:
        return HttpResponse("<h1>u r not logged in</h1>")
...
def applyjob(request):
    if request.user.is_authenticated and request.user.groups.filter(name='student').exists():
      ...
      if request.method == "POST":
        ...
        x = request.user.username
        b = StudentDetails.objects.filter(username=x)
        b = str(b[0].branch)
        print(sal,bon,b)
        if(b=="it"):
         y = JobPosition.objects.filter(salary__gte=sal,bond_years__lte=bon,information_technology="yes").order_by('salary')
        if (b == "cse"):
            y = JobPosition.objects.filter(salary__gte=sal, bond_years__lte=bon,cse="yes").order_by('salary')
        if (b == "me"):
            y = JobPosition.objects.filter(salary__gte=sal, bond_years__lte=bon,mech="yes").order_by('salary')
        if (b == "ce"):
            y = JobPosition.objects.filter(salary__gte=sal, bond_years__lte=bon,civil="yes").order_by('salary')
        if (b == "eee"):
            y = JobPosition.objects.filter(salary__gte=sal, bond_years__lte=bon,eee="yes").order_by('salary')
        if (b == "ece"):
            y = JobPosition.objects.filter(salary__gte=sal, bond_years__lte=bon,ece="yes").order_by('salary')
        if (b == "ch"):
            y = JobPosition.objects.filter(salary__gte=sal, bond_years__lte=bon,chemical="yes").order_by('salary')
        print(y)
        if(len(y)==0):
            s="no vacancies for this preference"
            print("failed",s)
            return render(request, 'portal/applyjob.html',{'s':s})
        else:
            return render(request, 'portal/applyjob.html', {'y': y, 's': s})
      else:
          return render(request, 'portal/applyjob.html', { 'y':y,'s': s})
    else:
        return HttpResponse("<h1>u r not logged in</h1>")
...
def apply(request,opt):
    if request.user.is_authenticated and request.user.groups.filter(name='student').exists():
        if request.method=="POST":
            x=request.user.username
            print(x)
            y=JobPosition.objects.filter(job_id=opt)[0].username
            job=AppliedJobs()
            job.student_id=x
            job.company_id=y
            job.job_id=opt
            ...
            c=JobPosition.objects.filter(job_id=opt)[0].username
            print(c)
            x=CompanyDetails.objects.filter(username=c)
            print(x)
            return render(request,'portal/compdisp.html',{'post':x[0]})
...
def selectstu(request):
    ...
    if request.user.is_authenticated and request.user.groups.filter(name='company').exists():
        if request.method == "POST":
             ...
             x=len(JobPosition.objects.filter(job_id=jobid,username=u))
             if(x==0):
                 s="enter correct job id"
                 return render(request, 'portal/sstu.html', {'y': y,'s':s})
             x=len(AppliedJobs.objects.filter(job_id=jobid,company_id=u))
             if(x==0):
             ...
             x = AppliedJobs.objects.filter(job_id=jobid,company_id=u).values('student_id')
             y=[]
             print(x)
             y=[]
             print("the total number is",len(y))
             for i in x:
                 b=StudentDetails.objects.filter(class_10_cgpa__gte=tenth,class_12_percentage__gte=twth,cgpa_Btech__gte=btech,username=i['student_id'])
                 if(b.count()>0):
                   y.append(b)
             print("the total number is",len(y))
             if(len(y)==0):
          ...
          return render(request, 'portal/sstu.html', {'y': y})
...
def stumail(request,opt):
    if request.user.is_authenticated and request.user.groups.filter(name='company').exists():
        if request.method=="POST":
            recv=StudentDetails.objects.filter(username=opt)[0].email
